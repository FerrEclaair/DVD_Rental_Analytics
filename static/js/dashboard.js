...
// ============================================================
// TIME SERIES FORECAST (Simple ML)
// ============================================================
(function setupForecast() {
  const runBtn = document.getElementById("ml-forecast-run");
  if (!runBtn) return;

  runBtn.addEventListener("click", async () => {
    const dataset = document.getElementById("ml-forecast-dataset").value;
    const horizon = parseInt(document.getElementById("ml-forecast-horizon").value) || 4;
    const summaryEl = document.getElementById("ml-forecast-summary");
    const metricsEl = document.getElementById("ml-forecast-metrics");

    summaryEl.innerHTML = "⏳ Sedang menghitung prediksi...";
    metricsEl.innerHTML = "";
    ...
    try {
      ...
      if (data.error) {
        summaryEl.innerHTML = `❌ Error: ${data.error}`;
        return;
      }

      // Render chart
      const historyX = data.history.map((d) => d.month);
      const historyY = data.history.map((d) => d.actual);
      const forecastX = data.forecast.map((d) => d.month);
      const forecastY = data.forecast.map((d) => d.predicted);

      // Connecting point (biar garis nyambung)
      const bridgeX = [historyX[historyX.length - 1], ...forecastX];
      const bridgeY = [historyY[historyY.length - 1], ...forecastY];
      ...
      const layout = {
        title: `Forecast: ${dataset.replace(/_/g, " ")}`,
        xaxis: { title: "Bulan" },
        yaxis: { title: "Nilai" },
        paper_bgcolor: "rgba(0,0,0,0)",
        plot_bgcolor: "rgba(0,0,0,0)",
      ...
      Plotly.newPlot("chart-ml-forecast", traces, layout, { responsive: true });

      // Render insight
      summaryEl.innerHTML = `<b>${data.insight}</b>`;

      // Render metrics
      metricsEl.innerHTML = `
        <div style="display:flex;gap:1.5rem;flex-wrap:wrap;margin-top:1rem;font-size:0.9rem;">
          <div>📊 <b>Model:</b> ${data.model}</div>
          <div>🎯 <b>R² Score:</b> ${data.metrics.r2_score}</div>
          <div>📅 <b>Data points:</b> ${data.metrics.data_points} bulan</div>
          <div>🔮 <b>Forecast:</b> ${data.metrics.horizon} bulan ke depan</div>
        </div>
      `;
    } catch (err) {
      summaryEl.innerHTML = `❌ Gagal request: ${err.message}`;
    }
  });
})();